...
建议1：取消 e.printStackTrace();  与 s_logger.error("CommonTree.generateTree.error", e) 功能重复
...
建议2： 取消 isErrorEnabled判断；isErrorEnabled判断是用来规避拼接错误信息耗时严重问题。 
...
建议1：规避eiBean可能为null时发生NullPointException
...
建议：使用slf4j代替log4j，commons-logging。
...
建议2：在catch中记录日志，尽可能的把日志记录上。尽可能合并return
...
建议：close函数请放入finally块，确保close不受write可能的异常而未执行。
...
建议：判断字符串是否未空或null，请使用 org.apache.commons.lang.StringUtils
...
建议：org.apache.commons.lang.BooleanUtils
